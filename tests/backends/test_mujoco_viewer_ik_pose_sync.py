...
def test_publish_snapshot_includes_selected_joint_world_pose() -> None:
    ...
    viewer._motions = [types.SimpleNamespace(num_frames=10, fps=30.0, num_dofs=1, joint_names=["joint_0"])]
    ...
    viewer._driver = types.SimpleNamespace(
        data=types.SimpleNamespace(
            ...
            xquat=np.array([[1.0, 0.0, 0.0, 0.0], [0.9238795, 0.0, 0.3826834, 0.0]], dtype=np.float64),
        ),
    ...
    np.testing.assert_allclose(snap.selected_joint_pos_m, np.array([0.25, -0.10, 0.55], dtype=np.float64))
    np.testing.assert_allclose(snap.selected_joint_quat_wxyz, np.array([0.9238795, 0.0, 0.3826834, 0.0]))
