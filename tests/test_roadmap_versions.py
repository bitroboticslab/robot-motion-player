...
def test_design_roadmap_marks_v050_as_full_gui_and_shifts_future_versions() -> None:
    ...
    assert "| v0.8.0  | Physics replay (PD control), deterministic tests" in text
    assert "| v0.9.0  | Isaac/NV backend, HDF5 support, plugin completion" in text
    assert "| v1.0.0  | Web/Jupyter viewer export, packaging/distribution hardening" in text
...
def test_summary_roadmap_phase_numbers_shifted_by_one_after_full_gui() -> None:
    ...
    assert "Phase 8 — Physics Replay + Advanced Evaluation (Milestone 0.8.0)" in text
    assert "Phase 9 — Multi-Backend & Plugin System (Milestone 0.9.0)" in text
    assert "Phase 10 — Distribution & Community (Milestone 1.0.0)" in text
